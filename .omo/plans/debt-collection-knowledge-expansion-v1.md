...
**Decisions to sanity-check:** 사용자가 Next Work 1-3, 즉 원격 MCP 배포/remote smoke/client docs를 이번 웨이브에서 건너뛰기로 했다. 추가 인터뷰보다는 조사 품질이 핵심이라고 정리했다. 이번 계획은 지식 확장만 다루며 execution boundary는 계속 non-executing으로 둔다.
...
- Treat legal research as MCP-first:
  - use Korean-law MCP for statute search, article text, applicable-law/effective-date checks, precedent/interpretation search, and citation verification;
  - write only summarized source metadata and source refs to evidence/resources.
- Treat finance judgment as source-research-first, not user-interview-first:
  - inspect available MCP surfaces first;
  - if no dedicated finance MCP is available, use official/public web sources and legal MCP results;
  - prefer official or primary sources such as statute/case-law records, court procedure pages, regulator guidance, and central-bank/public-rate references;
  - freeze any adopted finance rule or rate source into deterministic resources/tests rather than relying on live web calls.
...
- [ ] 6. Research finance-source channels and extend finance/human-review regressions
  What to do / Must NOT do: First inspect whether a finance-specific MCP or repo-local finance source exists. If none is available, use official/public web sources plus Korean-law MCP for statutory finance issues such as legal interest, maximum interest, payment allocation, enforcement costs, assignment/succession, guarantee/surety, and subrogation/reimbursement. Freeze the adopted source refs into evidence/resources, then add deterministic tests/evidence for finance and review outcomes that agents commonly over-assume: disputed balances, payment allocation conflicts, enforcement cost review, assignment/succession, guarantee/surety, subrogation/reimbursement candidate, and operator review rejection/needs-more-evidence flows. Do not turn fixture calculations into authoritative production accounting, do not scrape unofficial blogs as authority, and do not add live web/MCP calls to deterministic tests.
  Parallelization: Wave 2 | Blocked by: 2, 3 | Blocks: 7, 8, 9
  References (executor has NO interview context - be exhaustive): `resources/finance/claim_finance_model_v1.json`; `trustgraph_legal/finance_claims.py`; `tests/unit/legal_ontology/test_finance_claim_model_v1.py`; `resources/action_packets/debt_collection_action_packets_v1.json`; `trustgraph_legal/debtor_governance.py`; Korean-law MCP `search_law`, `get_law_text`, `legal_analysis`, `search_decisions`; official web sources discovered during planning: 국가법령정보센터/law.go.kr for civil/payment-allocation and interest authorities, 대한민국 법원 전자소송/ecfs.scourt.go.kr for civil execution procedure pages, 금융감독원/fss.or.kr for debt-collection/regulatory guidance, 한국은행/bok.or.kr for public rate references when relevant.
  Acceptance criteria (agent-executable): `/opt/homebrew/bin/python3 -m pytest tests/unit/legal_ontology/test_finance_claim_model_v1.py tests/unit/legal_ontology/test_action_packets_v1.py tests/unit/legal_ontology/test_debtor_governance.py -q` passes; evidence proves the finance-source channel decision, ambiguous finance cases, and rejected review records remain review-required.
  QA scenarios (name the exact tool + invocation): happy: write `.omo/evidence/debt-collection-knowledge-expansion-v1/task-6-finance-source-research.json` and `.omo/evidence/debt-collection-knowledge-expansion-v1/task-6-finance-review-happy.json`; failure: conflicting balance, unsupported source, or missing audit metadata writes `.omo/evidence/debt-collection-knowledge-expansion-v1/task-6-finance-review-failure.json`.
