...
- [x] 1. Define Debtor Context Graph v0 schema types and JSON contract
...
- [x] 2. Implement deterministic OCR page inventory and document assembly builder
...
- [x] 3. Add minimized OCR assembly fixtures and PII-safe real corpus probe
...
- [x] 4. Add route and legal rule-source resources with validators
...
- [x] 5. Expose DocumentAssembly CLI and manifest-to-assembly bridge
...
- [x] 6. Build DebtorContextGraph from assembly output and existing extractors
  ...
  Leader disposition after review: Implemented by H commit `611909b5` and integrated into master by merge `b6ca1f60`. I review accepted the builder contract, with follow-ups carried forward: route matcher must deliberately handle H's `accepted` fact status in Todo 9, snapshot IDs must include fact/version deltas in Todo 7, and hash-shaped PII false positives must be handled explicitly in the final evidence gate.

- [x] 7. Add snapshot replay, diff, and provenance validation
  ...
  Leader disposition: Implemented by J commit `e9fcb007` and integrated into master by merge `2f857d78`. Leader verification passed focused snapshot pytest, py_compile, basedpyright error-level checks, JSON evidence parsing, diff check, and clarified PII gate as `HASH_ONLY_FALSE_POSITIVES` with no matched values. L follow-up review accepted Todo 7 and confirmed provenance depth, derived evidence, legacy `graph_snapshot_id` collision reporting, and hash-only PII evidence gaps are closed.

- [x] 8. Implement route candidate matcher over DebtorContextGraph
  ...
  Leader integration note: Original Todo 8 commit `14ad71d9` is accepted and integrated. Later E follow-up commit `0465cc6e` broadens `accepted` facts as clear route facts, but is intentionally not integrated; the canonical master fix for H compatibility is H commit `6ee22757`, which emits route facts as `verified`. Future integrations must use explicit member ids and must not run whole-team integration while E contains the superseded follow-up branch tip.

- [x] 9. Integrate StopGate and legal rule-source metadata into graph route evaluation
  ...
  Leader disposition: Implemented by K commit `cb4ea60b` and integrated into master by merge `450d98cc`. Leader verification passed 23 route/StopGate/debtor-context tests, py_compile, JSON evidence parsing, diff check, size checks (`route_candidates.py` 246 pure LOC), and PII scan `NO_FINDINGS`. K kept `accepted` out of clear statuses, gated legal sources by `review_status`/`retrieval_status`/`effective_date`, scoped StopGate reason bridging by route family, preserved no-direct-execution output, and did not treat H summary case packets as legacy case-graph clearance.

- [x] 10. Add end-to-end debtor context CLI
  ...
  Leader disposition: Implemented by M commit `36633df6` and integrated into master by merge `13e7a097`. Leader verification passed the focused debtor-context pipeline tests (36 related tests on master), `/usr/bin/python3` route candidate import proof, py_compile, basedpyright, JSON evidence validation, and a real OCR summary-only CLI run over 208 pages. The real OCR CLI output had 18 route candidates, one review item, zero raw body keys, and zero source/path keys.

- [x] 11. Add additive MCP tools for debtor graph reading and route explanation
  ...
  Leader disposition: Implemented by N commit `5d68c0b3` and integrated into master by merge `40d49aa8`. It appends five `debtor_graph` MCP tools after the existing 16, preserves context-only auth, keeps repo-root path rejection redacted, and avoids creating new governance field names in route explanations. Leader and P verification passed focused MCP/fake-MCP tests, integrated MCP tests, JSON evidence validation, py_compile, basedpyright, diff check, and leak-marker checks. P accepted Todo 11 for contract/security scope.

- [x] 12. Add governance/review hooks for facts, assemblies, and routes
  ...
  Leader disposition: Implemented by O commit `3901c90b` and integrated into master by merge `880cba38`. Governance records are service-side only, leave ontology/routes/legal-source resources read-only, and cover unknown assemblies, identity unresolved, conflicting facts, legal-source review states, blocked routes, and manual fact review decisions. Leader and P verification passed focused governance tests, Python 3.9 import/py_compile, JSON evidence validation, PII scan `NO_FINDINGS`, size gates, diff check, and integrated MCP/governance tests. P accepted Todo 12 and the integrated N+O state.

- [x] 13. Add integration and fake-MCP contract tests
  ...
  Leader disposition: Integrated Q commit `836d2f46` via merge `8466c7a0`. Reproduced 19 MCP/pipeline tests, JSON evidence validation, diff check, and PII/path scan with no findings.

- [x] 14. Document Debtor Context Graph v0 operator/developer contract
  ...
  Leader disposition: Integrated R commit `5fe665a1` via merge `0e494679`. Reproduced focused docs/MCP smoke tests, required doc term checks, JSON evidence validation, diff check, and PII/path scan with no findings.

- [x] 15. Run real OCR eval pack and final evidence generation
  ...
  Leader disposition: Integrated S commit `29de4441`. Reproduced final focused pytest with 51 passed, final JSON validation, final PII/path scan with no findings, diff check, Python 3.9 compile spot-check, 21 MCP tools, redacted outside-root path failure, and real OCR summary counts: 208 pages, 1 assembly, 18 route candidates, 1 review item, 1 unknown assembly.
...
- [x] F1. Plan compliance audit
  ...
  - Leader disposition: Completed by S final evidence and accepted by T final review. T verified Todos 1-15 are integrated and final verification evidence exists.
- [x] F2. Code quality review
  ...
  - Leader disposition: Completed by S final evidence and accepted by T final review. Final closeout delta did not modify production Python; relevant compile and focused tests passed.
- [x] F3. Real manual QA
  ...
  - Leader disposition: Completed by S final evidence and accepted by T final review. Real OCR summary-only eval reported 208 pages, 1 assembly, 18 route candidates, 1 review item, and 1 unknown assembly.
- [x] F4. Scope fidelity
  ...
  - Leader disposition: Completed by S final evidence and accepted by T final review. Final PII/path scan returned no findings, existing 16 MCP tools remain first, five debtor graph tools are additive, and no deployment/runbook mutation was introduced by the closeout.
